...
exports.createOpportunity = (req, res) => {
  ...
  db.beginTransaction((err) => {
    ...
    Opportunity.createOpportunity(opportunityData, (err, result) => {
      ...
      // Get the travel_opportunity_id from the insert result
      const travelOpportunityId = result.insertId;
      ...
      Lead.updateLeadStatusOpp(leadid, 'opportunity', (err) => {
        ...
        Opportunity.updateCustomerStatus(customerid, 'existing', origincity, (err) => {
         ...
         // Only insert reminder if reminder_setting has a value
const commitTransaction = () => {
  db.commit((err) => {
    ...
    res.status(201).json({
      message: 'Opportunity + Reminder created successfully',
      opportunityId: result.insertId,
      travelOpportunityId: travelOpportunityId,
    });
  });
};
...
if (reminder_setting) {
  const reminderQuery = `
    ...
    (lead_id, travel_opportunity_id, reminder_setting, notes, user_id, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, NOW(), NOW())
  `;

  db.query(
    reminderQuery,
    [leadid, travelOpportunityId, reminder_setting, notes, user_id],
    (err) => {
      if (err) {
        console.error('Error inserting reminder setting:', err);
        return db.rollback(() => {
          res.status(500).json({ message: 'Failed to save reminder setting.' });
        });
      }
  ...
  // No reminder_setting provided, skip insertion
  commitTransaction();
}
          });
        });
