...
router.get("/", (req, res) => {
  const sql = "SELECT * FROM lead_status";
...
router.get("/all", (req, res) => {
  const sql = `
    ...
    FROM lead_status ls
    LEFT JOIN lead_status p ON ls.parent_id = p.id
  `;
...
router.post("/", (req, res) => {
  ...
  const sql = `
    INSERT INTO lead_status (name, is_primary, parent_id)
    VALUES (?, ?, ?)
  `;
...
router.delete("/:id", (req, res) => {
  ...
  db.query("DELETE FROM lead_status WHERE id=?", [id], (err) => {
    if (err) return res.status(500).json({ error: err });
