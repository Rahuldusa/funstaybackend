                    ...
                    fetch.on('message', (msg) => {
                        ...
                        msg.on('body', (stream) => {
                            simpleParser(stream, (err, parsed) => {
                                ...
                                const cleanText = parsed.text || parsed.html?.replace(/<[^>]+>/g, '') || '[No text content]';
                                            ...
                                            db.query(
                                                ...
                                                (err, sentEmails) => {
                                                    if (err || sentEmails.length === 0) {
                                                        console.error('No matching sent email or error for:', emailData.receiver_email);

                                                        // Still store notification with admin
                                                        storeNotification(null, emailData, { adminid: 'admin' }, () => {
                                                            processedCount++;
                                                            if (processedCount === uids.length) imap.end();
                                                        });
                                                        return;
                                                    ...
                                                    db.query(
                                                        ...
                                                        [leadId],
                                                        (err, leadDetails) => {
                                                            if (err || leadDetails.length === 0) {
                                                                console.error('Lead details fetch error or empty:', err);
                                                            ...
                                                            const leadInfo = leadDetails[0];
                                                            const notificationsToStore = [{ adminid: 'admin' }];
                                                            ...
                                                            notificationsToStore.forEach((recipient) => {
                                                                storeNotification(leadId, emailData, recipient, () => {
                                                                    notificationsStored++;
                                                                    if (notificationsStored === notificationsToStore.length) {
                                                                        processedCount++;
                                                                        if (processedCount === uids.length) imap.end();
                                                                    }
                                                                });
